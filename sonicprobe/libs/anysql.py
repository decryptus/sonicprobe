...
METHOD_ESCAPE = 3
METHOD_CAST = 4
METHOD_IS_CONNECTED = 5

LOG = logging.getLogger("sonicprobe.anysql")


def qmark_to_format(query, encoding = None):
    s   = []
    p   = ''
...
class cursor(object): # pylint: disable=useless-object-inheritance
    ...
    def __init__(self, conn, methods, log_reconnect = False):
        ...
        self.__dbapi2_cursor    = conn._get_raw_cursor()
        self.__methods          = methods
        self.__log_reconnect    = log_reconnect
    ...
    def escape(self, s):
        return self.__methods[METHOD_ESCAPE](s)
    ...
    def cast(self, fieldname, xtype):
        cast_ = self.__methods[METHOD_CAST]
        if cast_ is None:
            return fieldname

        return cast_(fieldname, xtype)
...
class connection(object): # pylint: disable=useless-object-inheritance
    """
    ...
    def is_connected(self, link = None):
        uri_is_connected_method = self.__methods[METHOD_IS_CONNECTED]

        return uri_is_connected_method(self.__dbapi2_conn, link)
...
def __compare_api_level(als1, als2):
    lst1 = list(map(int, als1.split('.')))
    lst2 = list(map(int, als2.split('.')))
    if lst1 < lst2:
